...
patches-own [ ; patches are landlords
  ...
  prev-mean-rent; variable: average rent in the neighborhood at t-1
...
to setup
  ...
  ask patches [
    ...
    set rent quality
    set prev-mean-rent mean [rent] of neighbors
...
to supply
  ask patches [
    let mean-rent mean [rent] of neighbors
    set quality b_inertia * quality + (1 - b_inertia) * mean-rent

    set pcolor quality * 10 + 22
    set prev-mean-rent mean-rent

    if any? turtles-on neighbors [
      set status-nb mean [status] of turtles-on neighbors
    ]
    if not any? turtles-on neighbors [set status-nb 0]
...
<experiments>
  <experiment name="Experiment" repetitions="10" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="b_inertia">
      <value value="0.8"/>
      <value value="0.95"/>
    </enumeratedValueSet>
