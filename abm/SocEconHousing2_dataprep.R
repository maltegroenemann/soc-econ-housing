...
#library(arrow) # TODO uncomment
library(nanoparquet) # TODO comment out, for local use, smaller than arrow
library(dplyr)
library(tidyr)
library(DescTools)

# TODO change path to prepare data from the respective experiment
# File paths as used on Helix HPC, not paths to local data files!
...
#data_directory <- "./data/gentri/"
#data_directory <- "./data/popdyn/"
...
t_min <- 100
if (data_directory == "./data/main/") t_min <- 300
if (data_directory == "./data/gentri/") t_min <- 200 # TODO
if (data_directory == "./abm/c_gentri/test/") t_min <- 80
...
unit_data <- unit_data %>% 
  mutate(nb = paste(floor(x / size_nb),
                    floor(y / size_nb), 
                    sep = "_"),
         empty = is.na(hh_id),
