...
city4_long %>%
  ...
  facet_grid(rows = vars(r_correlation),
             ...
             labeller = labeller(.rows = label_r, 
                                 .cols = label_a)) +
  labs(x = "", y = "Reardon Index", fill = "",
       title = "Residential Segregation, d = 0.95") +
  scale_x_discrete(breaks = NULL) +
  scale_fill_manual(values = label_seg) +
  theme_bw() +
...
city4_long %>%
  ...
  facet_grid(rows = vars(r_correlation),
             ...
             labeller = labeller(.rows = label_r, 
                                 .cols = label_a)) +
  labs(x = "", y = "Reardon Index", fill = "",
       title = "Residential Segregation, d = 0.8") +
  scale_x_discrete(breaks = NULL) +
  scale_fill_manual(values = label_seg) +
  theme_bw() +
  theme(legend.position = "bottom")
...
```{r reg_quality}
stargazer(lm(data = subset(unit_data, time == 150 & a_preferences == 0 & r_correlation == 0),
             quality ~ income + status), 
          lm(data = subset(unit_data, time == 150 & a_preferences == 0.5 & r_correlation == 0),
             quality ~ income + status), 
          lm(data = subset(unit_data, time == 150 & a_preferences == 1 & r_correlation == 0),
             quality ~ income + status),  
          lm(data = subset(unit_data, time == 150 & a_preferences == 0 & r_correlation == 0.5),
             quality ~ income + status),
          lm(data = subset(unit_data, time == 150 & a_preferences == 0.5 & r_correlation == 0.5),
             quality ~ income + status),
          lm(data = subset(unit_data, time == 150 & a_preferences == 1 & r_correlation == 0.5),
             quality ~ income + status),
          title = "Linear Regression on Housing Quality",
          style = "asr",
          type = "text",
          column.sep.width = "1pt",
          column.labels = c("a=0,r=0", "a=0.5,r=0", "a=1,r=0", "a=0,r=0.5", "a=0.5,r=0.5", "a=1,r=0.5"),
          dep.var.labels.include = FALSE,
          covariate.labels = c("Income", "Status"),
          omit.stat=c("f", "ser"))
```
...
```{r reg_mob}
stargazer(lm(data = subset(indiv_data, a_preferences == 0 & r_correlation == 0),
             prob_move_unit ~ income + status), 
          lm(data = subset(indiv_data, a_preferences == 0.5 & r_correlation == 0),
             prob_move_unit ~ income + status), 
          lm(data = subset(indiv_data, a_preferences == 1 & r_correlation == 0),
             prob_move_unit ~ income + status),  
          lm(data = subset(indiv_data, a_preferences == 0 & r_correlation == 0.5),
             prob_move_unit ~ income + status),
          lm(data = subset(indiv_data, a_preferences == 0.5 & r_correlation == 0.5),
             prob_move_unit ~ income + status),
          lm(data = subset(indiv_data, a_preferences == 1 & r_correlation == 0.5),
             prob_move_unit ~ income + status),
          title = "Linear Regression on the Probability of Moving",
          style = "asr",
          type = "text",
          column.sep.width = "1pt",
          column.labels = c("a=0,r=0", "a=0.5,r=0", "a=1,r=0", "a=0,r=0.5", "a=0.5,r=0.5", "a=1,r=0.5"),
